...
export function playbackSetupProgress(completed) {
  const count = Math.min(4, Math.max(0, Math.floor(Number(completed) || 0)));
  return { completed: count, total: 4, percent: count * 25, message: playbackSetupStages[count] || 'Playback ready' };
}
...
export async function readPlaybackSetup(response, onProgress) {
  ...
  function consume(line) {
    ...
    if (event.type === 'progress') onProgress?.(playbackSetupProgress(event.completed));
