...
import { hasEnglishAudioTrack, playbackTracks } from '../src/lib/server/playback-tracks.js';
...
test('unlabelled anime candidates require an English source audio track', () => {
  assert.equal(hasEnglishAudioTrack(playbackTracks([{ index: 1, codec_type: 'audio', tags: { language: 'jpn' } }, { index: 2, codec_type: 'audio', tags: { language: 'eng' } }])), true);
  assert.equal(hasEnglishAudioTrack(playbackTracks([{ index: 1, codec_type: 'audio', tags: { title: 'English dub' } }])), true);
  assert.equal(hasEnglishAudioTrack(playbackTracks([{ index: 1, codec_type: 'audio', tags: { handler_name: 'English stereo' } }])), true);
  assert.equal(hasEnglishAudioTrack(playbackTracks([{ index: 1, codec_type: 'audio', tags: { language: 'jpn' } }, { index: 2, codec_type: 'subtitle', tags: { language: 'eng' } }])), false);
});
test('live conversion rejects an unverified source without English audio', async () => {
  const { startHlsConversion } = await import('../src/lib/server/streamer.js');
  const job = { mode: 'cached-convert', sourcePath: '/unused/video.mkv', strategy: 'remux', requireEnglishTrack: true, events: [] };
  const inspect = () => ({ metadata: Promise.resolve({ tracks: [{ index: 1, type: 'audio', language: 'jpn', label: 'Japanese' }], audioIndex: 1 }), validated: Promise.resolve() });
  await assert.rejects(() => startHlsConversion(job, {}, 0, '/unused', () => {}, async () => ({}), inspect), error => error.code === 'NO_ENGLISH_AUDIO');
});
...
test('real multi-track source selects the requested language and converts text captions', {timeout:15000}, async () => {
  ...
  try{
    ...
    const job={mode:'cached-convert',sourcePath:input,strategy:'remux',release:'SDR',requireEnglishTrack:true,events:[]};
