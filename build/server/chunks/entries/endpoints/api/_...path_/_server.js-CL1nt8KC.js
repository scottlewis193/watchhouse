import { readdir, stat, rm, mkdir, mkdtemp, writeFile, readFile, rename } from 'node:fs/promises';
...
import { o as offlineMediaKey } from '../../../../chunks/offline.js-BZ0Jbt3M.js';
...
async function postedFileAvailable(file, settings) {
  ...
  try {
    ...
    const indexes = [.../* @__PURE__ */ new Set([Math.floor(file.segments.length / 2), file.segments.length - 1])];
...
function conversionSucceeded(code, stderr = "", bytes = 1) {
  return code === 0 && !String(stderr).trim() && bytes > 0;
}
...
function shouldCacheDirectPlayback(job) {
...
async function openPostedRangeServer(job, settings, connect = connectNntp) {
...
async function extractPostedArchive(directory, archives, names = archiveFilenames(archives), job) {
  ...
  if (/\.(?:rar|r\d\d)$/i.test(name)) await run("unrar", ["x", "-o+", "-idq", name], directory, job);
  else await run("7z", ["x", "-y", name, `-o${directory}`], directory, job);
}
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/play") {
      ...
      if (offline?.status === "ready") {
        ...
        const job2 = { id: randomUUID(), media: { ...media }, status: "ready", message: "Playing downloaded copy.", progress: 100, created: Date.now(), mode: local.mode, path: local.path, sourcePath: local.sourcePath, mime: local.mime, strategy: local.strategy, release: local.release || "", untaggedAudioTrack: Number(playbackSettings.untaggedAudioTrack) || 2 };
...
//# sourceMappingURL=_server.js-CL1nt8KC.js.map
